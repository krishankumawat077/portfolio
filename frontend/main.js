...
const API_BASE = window.location.hostname === 'localhost' ||
  ...
  : 'https://YOUR-BACKEND-NAME.onrender.com/api';  // ← UPDATE THIS after Step 6
...
const observer = new IntersectionObserver((entries) => {
  entries.forEach(entry => {
    if (entry.isIntersecting) {
      ...
      siblings.forEach((el, i) => { el.style.transitionDelay = `${i * 0.07}s`; });
...
joinForm.addEventListener('submit', async (e) => {
  ...
  try {
    ...
    if (res.ok) {
      setStatus('success', `🎉 Welcome to Campus Connect, ${data.fullName.split(' ')[0]}! You're now part of the community.`);
      joinForm.reset();
      selected.clear();
      ...
      setStatus('error', result.message || 'Something went wrong. Please try again.');
      if (result.errors) showErrors(result.errors);
    }
  } catch (err) {
    console.error(err);
    setStatus('error', '⚠️ Cannot reach the server. Is your backend running? Check the API_BASE URL in main.js.');
  } finally {
  ...
  try {
    ...
    members.forEach((m, i) => {
      ...
      const interestTags = m.interests
        ? m.interests.split(',').slice(0, 3).map(i =>
